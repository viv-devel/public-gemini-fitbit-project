        ...
        Buffer: {
            from: jest.fn((str, encoding) => {
                if (str === 'validStateBase64') {
                    return { toString: jest.fn(() => JSON.stringify({ firebaseUid: 'testFirebaseUid', redirectUri: 'http://example.com/redirect' })) };
                }
                if (str === 'stateWithoutRedirectUriBase64') {
                    return { toString: jest.fn(() => JSON.stringify({ firebaseUid: 'testFirebaseUid' })) };
                }
                if (str === 'stateWithoutFirebaseUidBase64') {
                    return { toString: jest.fn(() => JSON.stringify({ redirectUri: 'http://example.com/redirect' })) };
                }
                if (str === 'invalidJsonStateBase64') {
                    return { toString: jest.fn(() => 'invalid json') };
                }
                if (str === 'invalidBase64') { // このケースも追加
...
describe('fitbitWebhookHandler', () => {
    ...
    test('should throw error if GCP_PROJECT is not set', async () => {
        ...
        await expect(fitbitWebhookHandler({}, mockRes)).rejects.toThrow('GCP_PROJECT 環境変数が設定されていません。Secret Manager へのアクセスには必須です。');
    });

    test('should throw error if FITBIT_REDIRECT_URI is not set', async () => {
        delete process.env.FITBIT_REDIRECT_URI;
        await expect(fitbitWebhookHandler({}, mockRes)).rejects.toThrow('FITBIT_REDIRECT_URI 環境変数が設定されていません。');
    });
    ...
    test('should handle OPTIONS request', async () => {
        ...
        expect(mockRes.set).toHaveBeenCalledWith('Access-Control-Allow-Headers', 'Content-Type, Authorization');
    ...
    describe('GET request (OAuth callback)', () => {
        ...
        test('should exchange code for tokens and redirect if state is valid and includes redirectUri', async () => {
            ...
            expect(exchangeCodeForTokens).toHaveBeenCalledWith('testClientId', 'testClientSecret', 'testCode', 'testFirebaseUid');
            expect(mockRes.redirect).toHaveBeenCalledWith(302, 'http://example.com/redirect?uid=testFirebaseUid');
        });
        ...
        test('should exchange code for tokens and send success message if state is valid but no redirectUri', async () => {
            ...
            expect(mockRes.send).toHaveBeenCalledWith('Authorization successful! User UID: testFirebaseUid. You can close this page.');
        });
        ...
        test('should return 400 if state parameter is missing', async () => {
            ...
            expect(mockRes.json).toHaveBeenCalledWith({ error: 'Invalid request: state parameter is missing.' });
        });
        ...
        test('should return 400 if state parameter cannot be decoded', async () => {
            ...
            Buffer.from.mockImplementationOnce(() => { throw new Error('Invalid base64'); });

            await fitbitWebhookHandler(mockReq, mockRes);

            expect(mockRes.status).toHaveBeenCalledWith(400);
            expect(mockRes.json).toHaveBeenCalledWith({ error: expect.stringContaining('Invalid state: could not decode state parameter.') });
        });
        ...
        test('should return 400 if firebaseUid is missing from decoded state', async () => {
            ...
            expect(mockRes.status).toHaveBeenCalledWith(400);
            expect(mockRes.json).toHaveBeenCalledWith({ error: 'Invalid state: Firebase UID is missing.' });
        });
        ...
        test('should handle error during exchangeCodeForTokens', async () => {
            ...
            exchangeCodeForTokens.mockRejectedValueOnce(new Error('Fitbit API error'));
    ...
    describe('POST request (food logging)', () => {
        ...
        test('should log foods successfully with valid token', async () => {
            ...
            expect(processAndLogFoods).toHaveBeenCalledWith(mockTokens.accessToken, mockNutritionData, mockTokens.fitbitUserId);
            expect(mockRes.status).toHaveBeenCalledWith(200);
        ...
        test('should refresh token and log foods successfully if token is expired', async () => {
            ...
            expect(verifyFirebaseIdToken).toHaveBeenCalledWith(mockIdToken);
            expect(getTokensFromFirestore).toHaveBeenCalledWith(mockFirebaseUid);
            expect(refreshFitbitAccessToken).toHaveBeenCalledWith(mockFirebaseUid, 'testClientId', 'testClientSecret');
            expect(processAndLogFoods).toHaveBeenCalledWith('newAccessToken', mockNutritionData, mockExpiredTokens.fitbitUserId);
            expect(mockRes.status).toHaveBeenCalledWith(200);
            expect(mockRes.json).toHaveBeenCalledWith({
                message: 'All foods logged successfully to Fitbit.',
                loggedData: mockNutritionData,
        ...
        test('should return 401 if Authorization header is missing', async () => {
            ...
            expect(mockRes.json).toHaveBeenCalledWith({ error: 'Unauthorized: Authorization header is missing or invalid.' });
        });
        ...
        test('should return 401 if verifyFirebaseIdToken fails', async () => {
            verifyFirebaseIdToken.mockRejectedValueOnce(new AuthenticationError('Firebase auth error'));

            await fitbitWebhookHandler(mockReq, mockRes);

            expect(mockRes.status).toHaveBeenCalledWith(401);
            expect(mockRes.json).toHaveBeenCalledWith({ error: 'Firebase auth error' });
        });
        ...
        test('should return 400 if nutritionData is missing', async () => {
            ...
            expect(mockRes.status).toHaveBeenCalledWith(400);
            expect(mockRes.json).toHaveBeenCalledWith({ error: 'Invalid JSON body. Required: meal_type, log_date, log_time, and a non-empty "foods" array.' });
        });
        ...
        test('should return 401 if no tokens found for user', async () => {
            ...
            expect(mockRes.status).toHaveBeenCalledWith(401);
            expect(mockRes.json).toHaveBeenCalledWith({ error: `No tokens found for user ${mockFirebaseUid}. Please complete the OAuth flow.` });
        });

        test('should return 500 if refreshFitbitAccessToken fails', async () => {
            getTokensFromFirestore.mockResolvedValue(mockExpiredTokens);
            refreshFitbitAccessToken.mockRejectedValueOnce(new Error('Refresh failed'));
        ...
        test('should return 500 if fitbitUserId is missing from tokens', async () => {
            getTokensFromFirestore.mockResolvedValueOnce({ ...mockTokens, fitbitUserId: undefined });

            await fitbitWebhookHandler(mockReq, mockRes);

            expect(mockRes.status).toHaveBeenCalledWith(500);
            expect(mockRes.json).toHaveBeenCalledWith({ error: 'Fitbit user ID not found in the database.' });
        });

        test('should return 500 if processAndLogFoods fails', async () => {
            processAndLogFoods.mockRejectedValueOnce(new Error('Fitbit logging error'));

            await fitbitWebhookHandler(mockReq, mockRes);

            expect(mockRes.status).toHaveBeenCalledWith(500);
            expect(mockRes.json).toHaveBeenCalledWith({ error: 'Fitbit logging error' });
        });
    });
    ...
    test('should return 500 for unhandled errors', async () => {
        ...
        accessSecretVersion.mockRejectedValueOnce(new Error('Secret access failed'));

        await fitbitWebhookHandler(mockReq, mockRes);

        expect(mockRes.status).toHaveBeenCalledWith(500);
        expect(mockRes.json).toHaveBeenCalledWith({ error: 'Secret access failed' });
    });
});